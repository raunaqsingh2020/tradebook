...
const socket = io("http://localhost:8081", { transport: ["websocket"] });
// const socket = io("https://textbookbackend.herokuapp.com", { transport: ["websocket"] });

export async function registerAccount(username, password, question, answer) {
    try {
...
export async function getNotification(userId) { //IDEALLY ALL IN CAPS 
    try {
        ...
        return 'could not get notification';
    }
}
